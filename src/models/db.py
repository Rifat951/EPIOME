...
class dbEmbeddings(dbEmbeddings):
	"""
...
class DistillBertModel(DistillBertModel):
	"""
	This class overrides :class:`~transformers.BertModel`. Please check the
	superclass for the appropriate documentation alongside usage examples.
	"""

	config_class = DistillBertConfig
	...
	def __init__(self, config):
		...
		self.embeddings = DistillBertEmbeddings(config)
		self.init_weights()
...
class RobertaForMaskedLM(BertPreTrainedModel):
	...
	def __init__(self, config):
		...
		self.distilbert = DistillBertModel(config)
		self.lm_head = DistillBertLMHead(config)
...
class DistillBertLMHead(nn.Module):
	"""Roberta Head for masked language modeling."""

	def __init__(self, config):
		super().__init__()
		self.dense = nn.Linear(config.hidden_size, config.hidden_size)
		self.layer_norm = DistillBertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class DsitllBertForSequenceClassification(DistillBertPreTrainedModel):
	config_class = DistillBertConfig
	pretrained_model_archive_map = DISTILLBERT_PRETRAINED_MODEL_ARCHIVE_MAP
	base_model_prefix = "distillbert"
	...
	def __init__(self, config):
		...
		self.distillbert = DisitllBertModel(config)
		self.classifier_ER = DistillBertClassificationHead(config)
		self.classifier_IP = DistillBertClassificationHead(config)
		self.classifier_EX = DistillBertClassificationHead(config)
...
class DistillBertForMultipleChoice(DistillBertPreTrainedModel):
	config_class = DistillBertConfig
	pretrained_model_archive_map = DISTILLBERT_PRETRAINED_MODEL_ARCHIVE_MAP
	base_model_prefix = "distillbert"
	...
	def __init__(self, config):
		...
		self.distilbert = DisitllBertModel(config)
...
class DistillBertForTokenClassification(BertPreTrainedModel):
	...
	def __init__(self, config):
		...
		self.disitllbert = DisitllBertModel(config)
		self.dropout = nn.Dropout(config.hidden_dropout_prob)
